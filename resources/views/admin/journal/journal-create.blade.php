...
@section('content')
    <div class="max-w-5xl">
        ...
        <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            <form action="{{ $journal ? route('admin.journal.update', $journal->id) : route('admin.journal.store') }}"
                method="POST" enctype="multipart/form-data" class="grid gap-6 md:grid-cols-2">
                @csrf
                <div>
                    ...
                    <select name="subject_id"
                        ...
                        @foreach ($subjects as $subject)
                            <option value="{{ $subject->id }}"
                                {{ old('subject_id', $journal->subject_id ?? '') == $subject->id ? 'selected' : '' }}>
                                {{ $subject->title }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="nlm_id" value="{{ old('nlm_id', $journal->nlm_id ?? '') }}"
                        class="w-full rounded-lg border border-slate-300 px-4 py-3 outline-none focus:border-blue-500"
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="impact_factor"
                        ...
                        class="w-full rounded-lg border border-slate-300 px-4 py-3 outline-none focus:border-blue-500"
                        placeholder="Enter impact factor" required>
                </div>
                ...
                <div>
                    ...
                    <select name="is_active"
                        class="w-full rounded-lg border border-slate-300 px-4 py-3 outline-none focus:border-blue-500">
                        <option value="1" {{ old('is_active', $journal->is_active ?? 1) == 1 ? 'selected' : '' }}>
                            Active</option>
                        <option value="0" {{ old('is_active', $journal->is_active ?? 1) == 0 ? 'selected' : '' }}>
                            Inactive</option>
                    </select>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    @if (!empty($journal?->image))
                        <div class="mt-4">
                            <img src="{{ asset($journal->image) }}" alt="Journal Image"
