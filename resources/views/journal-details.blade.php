                    ...
                    <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                        ...
                        <div class="space-y-[1px] bg-slate-200">
                            @foreach ($menus as $item)
                                <a href="{{ $item->menu_type == 'custom' ? $item->menu_link : route('journal.menu.page', [$journal->id, $item->id]) }}"
                                    class="block px-5 py-3 text-lg text-white transition hover:bg-[#163d7a] 
       {{ $activeMenu && $activeMenu->id == $item->id ? 'bg-[#163d7a]' : 'bg-[#1f4f9d]' }}">
                    ...
                    <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                        ...
                        <div class="mt-6 space-y-6 text-[17px] leading-9 text-slate-800">
                            ...
                            @if ($page && $page->long_description)
                                <div
                                    class="prose max-w-none prose-headings:text-slate-800 prose-p:text-slate-700 prose-li:text-slate-700">
                                    ...
                                    @foreach ($eboards as $member)
                                        <div
                                            ...
                                            <div class="flex flex-col gap-5 md:flex-row md:items-start md:justify-between">
                                                ...
                                                <div class="shrink-0">
                                                    <img src="{{ $member->image ? asset($member->image) : 'https://via.placeholder.com/130x130?text=Profile' }}"
                                                        alt="{{ $member->name }}"
                                ...
                                <div class="space-y-6">
                                    @forelse($archiveGroups as $group)
                                        <div class="overflow-hidden rounded-2xl border border-cyan-300 bg-white shadow-sm">
                                            <div
                                                class="bg-gradient-to-r from-cyan-500 to-cyan-600 px-4 py-3 text-2xl font-semibold italic text-white">
                                                Year: {{ $group['year'] }}
                                            </div>
                                    ...
                                    @forelse($earlyViewArticles as $article)
                                        @php
                                            $googleScholarUrl =
                                                ...
                                                urlencode($article->paper_title);
                                            $googleSearchUrl =
                                                'https://www.google.com/search?q=' .
                                                urlencode($article->paper_title . ' ' . $article->author_name);
                                            $doiUrl = $article->doi
                                                ? 'https://doi.org/' . ltrim($article->doi, '/')
                                                : null;
                                        @endphp

                                        <div
                                            class="overflow-hidden rounded-2xl border border-cyan-400 bg-white shadow-sm transition hover:shadow-md">
                                            <div
                                                class="flex items-center justify-between bg-gradient-to-r from-cyan-500 to-cyan-600 px-4 py-3 text-white">
                                                <span
                                                    class="rounded-full bg-white/15 px-3 py-1 text-lg font-semibold italic">
                                                ...
                                                @if ($article->doi)
                                                    <div class="mb-4">
                                                        <a href="{{ $doiUrl }}" target="_blank"
                                                            ...
                                                            <span
                                                                class="inline-flex h-7 w-7 items-center justify-center rounded-full bg-amber-500 text-white text-xs font-bold">
                                                                DOI
                                                            </span>
                                                ...
                                                <div class="mt-5 flex flex-wrap items-center gap-2">
                                                    @if ($article->upload_doc)
                                                        <a href="{{ asset('storage/' . $article->upload_doc) }}"
                                                            target="_blank" title="View PDF / Document"
                                                            ...
                                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
                                                                ...
                                                                <path
                                                                    d="M7 2h7l5 5v13a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2zm7 1.5V8h4.5" />
                                                                <path
                                                                    d="M8 13h2.2a1.8 1.8 0 0 1 0 3.6H9.2V19H8v-6zm1.2 1.1v1.4h.9a.7.7 0 1 0 0-1.4h-.9zM13 13h1.9c1.4 0 2.4 1 2.4 2.8S16.3 19 14.9 19H13v-6zm1.2 1.1v3.8h.6c.8 0 1.3-.6 1.3-1.9s-.5-1.9-1.3-1.9h-.6z" />
                                                            </svg>
                                                        </a>
                                                    @endif

                                                    @if ($article->upload_link)
                                                        <a href="{{ $article->upload_link }}" target="_blank"
                                                    ...
                                                    @if ($article->doi)
                                                        <a href="{{ $doiUrl }}" target="_blank" title="Open DOI"
                                                    ...
                                                    <a href="{{ $googleSearchUrl }}" target="_blank"
                                                        ...
                                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
                                                            fill="currentColor" viewBox="0 0 24 24">
                                                            <path
                                                                d="M21.35 11.1H12v2.98h5.38c-.23 1.5-1.76 4.4-5.38 4.4-3.24 0-5.88-2.68-5.88-5.98s2.64-5.98 5.88-5.98c1.84 0 3.07.79 3.77 1.46l2.58-2.5C16.7 3.96 14.6 3 12 3 6.98 3 3 7.03 3 12s3.98 9 9 9c5.2 0 8.65-3.65 8.65-8.8 0-.6-.06-.86-.15-1.1z" />
                                                        </svg>
                                                    </a>
                                                </div>
                                            </div>
                            ...
                            @if ($page && $page->show_issue_volume && Str::contains($pageKey, 'current-issue'))
                                @if ($latestCurrentIssue)
                                    <div
                                        class="mb-4 rounded-xl bg-slate-100 px-4 py-3 text-lg font-semibold text-slate-700">
                                    ...
                                    @forelse($currentIssueArticles as $article)
                                        ...
                                        <div
                                            ...
                                            <div
                                                ...
                                                <span
                                                    class="text-base font-semibold uppercase tracking-wide">Article</span>
                                            </div>
                ...
                <aside class="space-y-5 lg:col-span-3">
                            ...
                            <div class="space-y-3 text-sm leading-7 text-slate-700">
                                ...
                                @else
                                    <p class="text-[16px] leading-7 text-slate-600">
                                        Explore recent papers, article updates, and related research topics through quick
                                        search links.
                                    </p>
                                @endif
                            </div>
                    ...
                    <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                        ...
                        <p class="mt-3 leading-7 text-slate-600">
                            Browse published articles, search research records, open document files, and access quick
